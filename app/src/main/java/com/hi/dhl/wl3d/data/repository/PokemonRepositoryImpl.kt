...
//    val mapper2InfoModel: Mapper<PokemonInfoEntity, PokemonInfoModel>
    val mapperfromfavorite: Mapper<FavoriteImageEntity, PokemonItemModel>
) : Repository {
...
//    }
    ...
    override fun getFavoriteMovies() : Flow<PagingData<PokemonItemModel>> {
        return Pager(pageConfig) {
            // 加载数据库的数据
//            db.pokemonDao().pokemonInfoByParameter(parameter)
    ...
    override suspend fun checkMovie(createdAt: Long): Boolean {
        val favoriteImageDao = db.favoriteImageDao()
        return favoriteImageDao.checkMovie(createdAt) > 0
    }
